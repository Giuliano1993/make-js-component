...
import makeAngular from './make-angular.mjs';
...
const createComponent = (componentName, framework, template, customFolder = "") => {
	...
	const filePath = path.join(configs.INIT_PATH, 'src', configs.STUBS_DIR, framework, template);

	fs.readFile(filePath, 'utf8', (err, data) => {
		// Make angular component
		if (framework === 'Angular') {
			makeAngular(filePath, componentName, customFolder);
		} else {
